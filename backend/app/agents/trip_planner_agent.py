...
**重要:保留位置信息**
1. 搜索结果中每个景点都包含location字段(经纬度,格式如"116.397128,39.916527")，**必须原样保留，不要丢弃**
2. 在总结景点信息时，用"经纬度: xxx"格式标注每个景点的经纬度坐标
3. 不要将经纬度四舍五入或简化，要完整保留
...
**重要:保留位置信息**
1. 搜索结果中每个酒店都包含location字段(经纬度,格式如"116.397128,39.916527")，**必须原样保留，不要丢弃**
2. 在总结酒店信息时，用"经纬度: xxx"格式标注每个酒店的经纬度坐标
3. 不要将经纬度四舍五入或简化，要完整保留
...
class MultiAgentTripPlanner:
    ...
    def __init__(self):
        ...
        try:
            ...
            if MCPTool is not None:
                ...
                print("  - 创建共享MCP工具...")
                import os
                wrapper_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "amap-wrapper.py")
                self.amap_tool = MCPTool(
                    name="amap",
                    description="高德地图服务",
                    server_command=["/opt/anaconda3/envs/helloagents/bin/python", wrapper_path],
    ...
    def _extract_location(cls, text: str) -> str:
        """提取经纬度信息，格式如 '116.397128,39.916527'"""
        patterns = [
            r"经纬度[:：]\s*([\d.]+,\s*[\d.]+)",
            r"坐标[:：]\s*([\d.]+,\s*[\d.]+)",
            r"location[:：]\s*([\d.]+,\s*[\d.]+)",
    ...
    def _compress_attractions_for_planner(cls, text: str, limit: int = 6) -> tuple[str, int]:
        ...
        for block in cls._split_ranked_blocks(text):
            ...
            location = cls._extract_location(block)
            poi_type = cls._extract_type(block)
            if not name:
                continue
            parts = [f"名称: {name}"]
            if address:
    ...
    def _compress_hotels_for_planner(cls, text: str, limit: int = 4) -> tuple[str, int]:
        ...
        for block in cls._split_ranked_blocks(text):
            ...
            location = cls._extract_location(block)
            rating = cls._extract_rating(block)
            if not name:
                continue
            parts = [f"名称: {name}"]
            if address:
...
4. 根据景点经纬度智能规划游玩顺序，同一天内的景点应按地理位置就近排序
5. 返回完整的JSON格式数据
6. 景点的经纬度坐标要真实准确，优先使用景点信息中提供的经纬度
...
"""
    ...
    def _compact_plan_to_trip_plan(
        ...
        for day_index in range(request.travel_days):
            ...
            # 解析酒店位置和评分
            hotel_location_str = hotel_choice.get("经纬度", "")
            hotel_rating_str = hotel_choice.get("评分", "")
            try:
                if "," in hotel_location_str:
                    hotel_lng, hotel_lat = hotel_location_str.split(",")
                    hotel_location = Location(longitude=float(hotel_lng.strip()), latitude=float(hotel_lat.strip()))
                else:
                    hotel_location = None
            except (ValueError, TypeError):
                hotel_location = None
            try:
                hotel_rating = float(hotel_rating_str) if hotel_rating_str else None
            except (ValueError, TypeError):
                hotel_rating = None
            # 根据评分估算酒店价格
            if hotel_rating:
                estimated_cost = int(200 + hotel_rating * 100)
            else:
                estimated_cost = 400
            ...
            hotel = Hotel(
                ...
                rating=hotel_rating_str if hotel_rating_str else "",
            )
            ...
            for idx, item in enumerate(selected_items[:2]):
                # 解析经纬度
                location_str = item.get("经纬度", "")
                try:
                    if "," in location_str:
                        lng, lat = location_str.split(",")
                        location = Location(longitude=float(lng.strip()), latitude=float(lat.strip()))
                    else:
                        location = Location(
                ...
                # 根据景点类型估算门票价格
                poi_type = item.get("类型", "")
                if "博物馆" in poi_type or "纪念馆" in poi_type:
                    ticket_price = 50
                elif "公园" in poi_type or "广场" in poi_type:
                    ticket_price = 20
                elif "寺庙" in poi_type or "宗教" in poi_type:
                    ticket_price = 30
                elif "景区" in poi_type or "保护区" in poi_type:
                    ticket_price = 80
                elif "游乐" in poi_type or "主题" in poi_type:
                    ticket_price = 150
                else:
                    ticket_price = 0
                ...
                attraction_models.append(
                    Attraction(
                        ...
                        category=item.get("类型", request.preferences[0] if request.preferences else "景点"),
